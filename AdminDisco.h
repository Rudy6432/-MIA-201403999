...
int mkdisk(int size, char unit, char *path, char *name)
{
    ...
    char *arch = (char *) malloc(1 + strlen(path)+ strlen(name));
    strcpy(arch, path);
    strcat(arch, name);
    ...
    FILE *disco = fopen("/home/mis discos/disk2.dsk","wb+");
...

{
    ...
    if(strcmp(name,mbr.particion1.nombre)==0)
    {
        if(strcmp(delete, "Fast") || strcmp(delete, "Full"))
        {
            mbr.particion1.tamanio=0;
            fseek(archivo,0,SEEK_SET);
            fwrite(&mbr, sizeof_mbr, 1, archivo);

            if(strcmp(delete, "Full"))
            {
                char buffer[1];
                int i=0;
    ...
    if(strcmp(name,mbr.particion2.nombre)==0)
    {
        if(strcmp(delete, "Fast") || strcmp(delete, "Full"))
        {
            mbr.particion2.tamanio=0;
    ...
    if(strcmp(name,mbr.particion3.nombre)==0)
    {
        if(strcmp(delete, "Fast") || strcmp(delete, "Full"))
        {
            mbr.particion3.tamanio=0;
    ...
    if(strcmp(name,mbr.particion4.nombre)==0)
    {
        if(strcmp(delete, "Fast") || strcmp(delete, "Full"))
        {
            mbr.particion4.tamanio=0;
